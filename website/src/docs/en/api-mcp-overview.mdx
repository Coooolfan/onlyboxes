...
The Console exposes four credential types for different use cases:

| Item | Cookie session | API Key (Bearer) | Execution Token (Bearer) | JIT Token (Bearer) |
|------|---------------|-----------------|---------------------------|--------------------|
| Credential type | Cookie `onlyboxes_console_session` | `Authorization: Bearer obxk_*` | `Authorization: Bearer obx_*` | `Authorization: Bearer obx_jit_v1.*` |
| Origin | `POST /api/v1/console/login` | `POST /api/v1/console/api-keys` (cookie only) | `POST /api/v1/console/tokens` | Externally signed (HMAC-SHA256, `CONSOLE_JIT_SIGNING_KEY`) |
| Lifetime | 12 hours (in-memory; invalidated on console restart) | Persistent until manually deleted | Persistent until manually deleted | Stateless; revoke by rotating the signing key |
| Applicable endpoints | All management endpoints | Most management endpoints (read + selective write) | `/api/v1/commands/*`, `/api/v1/tasks*`, `/mcp` | `/api/v1/commands/*`, `/api/v1/tasks*`, `/mcp` |
| Primary use | Web UI management | Programmatic management | Command execution and MCP | Third-party integrations without pre-created accounts |
...
>
> JIT Tokens are explicitly **rejected** on all dashboard/management routes.
...
JIT (Just-in-Time) Tokens allow external systems to authenticate on behalf of arbitrary users without pre-creating accounts. The console verifies the token signature and automatically provisions a non-admin, dashboard-login-disabled account on first use.

**Prerequisites:** Set `CONSOLE_JIT_SIGNING_KEY` on the console. If unset, all JIT tokens are rejected.

**Applicable endpoints:** Same as Execution Token — `/api/v1/commands/*`, `/api/v1/tasks*`, `/mcp`.
...
| Payload | Base64url (no padding) of JSON `{"iss":"<issuer>","sub":"<subject>"}` |
| Signature | Base64url (no padding) of HMAC-SHA256 over `obx_jit_v1.<payload>` |

### Expected Flow

1. **Configure** — Set `CONSOLE_JIT_SIGNING_KEY` on the Console; distribute the same key to all systems that will issue JIT tokens.
2. **Sign** — The caller encodes `{"iss":"...","sub":"..."}` as a Base64url payload and signs `obx_jit_v1.<payload>` with HMAC-SHA256.
3. **Request** — Send `Authorization: Bearer obx_jit_v1.<payload>.<sig>` to any command execution endpoint.
4. **Verify** — The Console checks the HMAC signature; returns `401` if the key is unset or the signature is invalid.
5. **Provision (first use only)** — The Console derives a deterministic `acc_jit_*` account from `(iss, sub)` and creates it with disabled dashboard credentials if it does not yet exist. Subsequent calls with the same pair skip this step.
6. **Execute** — The request proceeds under that account's ownership, subject to normal per-account resource rules.

> **Account isolation:** each `(issuer, subject)` pair maps to a separate account. Tokens with the same `iss` but different `sub` values are independent.
>
> **Dashboard access:** JIT-created accounts own execution resources, but they cannot log in to the dashboard with a password.
>
> **Revocation:** rotate `CONSOLE_JIT_SIGNING_KEY` to immediately invalidate all previously issued JIT tokens.
