...
The Console exposes two auth paths for different use cases:

| Item | Dashboard API (Cookie session) | Open API (Bearer token) |
|------|-------------------------------|-------------------------|
| Credential type | Cookie `onlyboxes_console_session` | `Authorization: Bearer <token>` |
| Origin | Created by `POST /api/v1/console/login` | Created via the Dashboard API Token management page |
| Lifetime | 12 hours (in-memory; invalidated on console restart) | Persistent until manually deleted |
| Applicable endpoints | `/api/v1/**` | `/api/v1/**` except `/api/v1/console/password` |
| Primary use | Account, token, worker management. For Web UI | Programmatic access |

> If no tokens exist in the system, all Bearer-authenticated endpoints return `401` by design.
