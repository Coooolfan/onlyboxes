...
JIT (Just-in-Time) Tokens allow external systems to authenticate on behalf of arbitrary users without pre-creating accounts. The console verifies the token signature and automatically provisions a non-admin account on first use.
...
5. **Provision (first use only)** — The Console derives a deterministic `acc_jit_*` account from `(iss, sub)` and creates it if it does not yet exist. Subsequent calls with the same pair skip this step.
