...
| Field | Type | Required | Constraints |
|-------|------|----------|-------------|
| `session_id` | string | Yes | Session ID; pass the string `"computerUse"` to route to the account's own `worker-sys` |
| `file_path` | string | Yes | Absolute path to the target file |
| `timeout_ms` | integer | No | `1..600000`, default `60000` |
...
- When `session_id` is `"computerUse"`, routes to the caller-owned `worker-sys` `readImage` capability with `action="export"`.
...
| `computerUse` account has no `worker-sys` | JSON-RPC error `-32010` with `data.error_code="WORKER_SYS_REQUIRED"` |
| `computerUse` account has a registered but offline `worker-sys` | JSON-RPC error `-32011` with `data.error_code="WORKER_SYS_OFFLINE"` |
