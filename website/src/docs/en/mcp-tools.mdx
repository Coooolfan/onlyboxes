...
| `exportFile` | Export a terminal-session file to object storage | Docker-backed terminal sessions only |
...
Exports a file from a Docker-backed terminal session to the console's configured S3-compatible object store and returns a presigned download URL.

**Input:**

| Field | Type | Required | Constraints |
|-------|------|----------|-------------|
| `session_id` | string | Yes | Terminal session ID returned by `terminalExec`; the special value `"computerUse"` is not supported |
| `file_path` | string | Yes | Absolute path to the target file inside the terminal session |
| `timeout_ms` | integer | No | `1..600000`, default `60000` |
...
- Routes to `terminalResource` with `action="export"` on the same Docker worker that owns the terminal session.
- The worker first probes the file via `docker exec <container> python3 -c ...`, then copies it out with `docker cp`, uploads it through a presigned HTTP `PUT`, and returns a presigned download URL.
...
| Field | Type | Description |
|-------|------|-------------|
| `signed_url` | string | Presigned download URL for the exported object |
...
- `session_id == "computerUse"` is rejected.
