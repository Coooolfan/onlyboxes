...
> `terminalExec`, `readImage`, and `exportFile` all rely on the terminal execution image. If that image does not include `python3`, file probe operations will fail before metadata can be validated or exported.
...
## `terminalResource` behavior

- Supports `validate`, `read`, and `export`.
- `read` enforces `WORKER_TERMINAL_OUTPUT_LIMIT_BYTES`.
- `export` is used by the MCP `exportFile` tool. It probes the file, copies it out of the container with `docker cp`, then uploads it through a presigned HTTP `PUT`.
- Non-JSON probe failures from `docker exec` / OCI startup are surfaced as docker exec errors with exit code and output summary instead of a generic JSON parse error.
