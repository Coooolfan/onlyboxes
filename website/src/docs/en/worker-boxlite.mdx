...
- boxlite runtime installed on the host
- Network access to the control node gRPC endpoint

## Starting the worker

Download the latest `worker-boxlite` binary from the [GitHub releases page](https://github.com/Coooolfan/onlyboxes/releases/latest), then run:
...
| `WORKER_TERMINAL_EXEC_BOXLITE_IMAGE` | No | `coolfan1024/onlyboxes-default-worker:0.0.3` | Image for terminal execution |
