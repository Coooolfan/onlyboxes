...
- Route access does not renew the Sandbox lease. Long-running previews must renew the terminal session explicitly with `POST /api/v1/sessions/:session_id/renew`; renewing the session does not extend the route's own expiry.
