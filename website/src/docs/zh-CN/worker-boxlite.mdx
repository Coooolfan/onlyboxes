...
全部可用配置请参考[带注释的配置模板](https://github.com/Coooolfan/onlyboxes/blob/main/worker/worker-boxlite/config.example.toml)。
...
例如将 terminalExec 和 terminalResource 上限都设为 `10`、session 上限设为 `2` 时，整个 worker 最多可同时处理 10 个 terminalExec 和 10 个 terminalResource，但任一 session 内两类操作合计不能超过 2。超出 session 上限返回 `409 session_busy`，对应能力的 worker 级容量耗尽则返回 `429 no_capacity`。
