...
| `WORKER_TERMINAL_EXEC_DOCKER_IMAGE` | 否 | `coolfan1024/onlyboxes-default-worker:0.0.3` | 终端执行镜像 |

> `terminalExec`、`readImage` 和 `exportFile` 都依赖 terminal 执行镜像。如果该镜像不包含 `python3`，文件探针会在读取或导出元数据之前失败。
...
- 支持 `validate`、`read` 和 `export`。
- `read` 会受 `WORKER_TERMINAL_OUTPUT_LIMIT_BYTES` 限制。
- `export` 由 MCP `exportFile` 工具调用；worker 会先探测文件，再通过 `docker cp` 将文件拷出容器，并用预签名 HTTP `PUT` 上传。
- 来自 `docker exec` / OCI 启动阶段的非 JSON 探针失败，会直接以带退出码和输出摘要的 docker exec 错误返回，而不是泛化成 JSON 解析错误。
