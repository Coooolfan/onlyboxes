...
Console 提供两套鉴权路径，覆盖不同使用场景：

| 项目 | 仪表盘 API（Cookie 会话） | 开放 API（Bearer Token） |
|------|--------------------------|--------------------------|
| 凭据类型 | Cookie `onlyboxes_console_session` | `Authorization: Bearer <token>` |
| 来源 | `POST /api/v1/console/login` 创建 | 通过仪表盘在 API Token 管理页创建 |
| 有效期 | 12 小时（内存态，console 重启后全部失效） | 持久，直到手动删除 |
| 适用端点 | `/api/v1/**` | `/api/v1/**` 但除 `/api/v1/console/password` 意外  |
| 主要用途 | 账号、token、worker 管理。适用于 Web UI | 程序化调用 |

> 如果系统中没有任何 token，所有 Bearer 鉴权接口将返回 `401`。
