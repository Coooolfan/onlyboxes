...
> 以下端点**仅接受 Cookie 会话**，拒绝 API Key Bearer 与 Dashboard JIT：`POST /api/v1/auth/password`、`POST /api/v1/api-keys`、`DELETE /api/v1/api-keys/:api_key_id`，以及所有 `/api/v1/tokens*` 端点。
>
