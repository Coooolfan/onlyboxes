...
JIT（Just-in-Time）Token 允许外部系统以任意用户身份进行认证，无需提前创建账户。Console 验证签名后，首次使用时自动派生并创建禁用 Dashboard 登录的非管理员账户。
...
5. **账户创建（仅首次）** — Console 由 `(iss, sub)` 确定性推导出 `acc_jit_*` 账户 ID，若不存在则使用禁用的 Dashboard credential 自动创建。后续相同组合的调用跳过此步。
6. **执行** — 请求在该账户的所有权下执行，受正常的账户级资源规则约束。

> **账户隔离：** 每个 `(issuer, subject)` 组合对应独立账户，相同 `iss` 不同 `sub` 的 Token 互不影响。
>
> **Dashboard 访问：** JIT 创建的账户只用于执行资源归属，不能通过密码登录 Dashboard。
>
