...
| 变量 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `CONSOLE_HIDDEN_TOOLS` | 否 | | 逗号分隔的 MCP capability ID 隐藏列表（`echo`、`pythonExec`、`terminalExec`、`computerUse`、`readImage`、`exportFile`）。被隐藏的工具不会出现在 `tools/list` 中，但如果客户端已知工具名，仍可继续调用。 |
