...
`CU:` 前缀大小写敏感，可通过 `CONSOLE_COMPUTER_USE_SESSION_ID_PREFIX` 修改。修改会改变 session ID 的解释方式，通常不建议修改。旧公开别名 `computerUse` 不再具有特殊含义；客户端必须先列出/选择 Worker，再使用其 ID。Console 发给 `worker-sys` 的内部值仍保持 `session_id="computerUse"`。
