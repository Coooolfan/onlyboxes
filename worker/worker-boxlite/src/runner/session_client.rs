...
mod tests {
    ...
    use crate::proto::registryv1::{
        connect_request, connect_response, worker_registry_service_server::WorkerRegistryService,
        worker_registry_service_server::WorkerRegistryServiceServer, CommandResult, ConnectAck,
        ConnectHello, HeartbeatAck,
    };
    ...
    impl CommandResultBuilder for FakeCommandResultBuilder {
        async fn build(&self, _cfg: &Config, dispatch: CommandDispatch) -> ConnectRequest {
            ...
            ConnectRequest {
                payload: Some(connect_request::Payload::CommandResult(CommandResult {
    ...
    impl WorkerRegistryService for FakeRegistry {
        ...
        async fn connect(
            &self,
            request: Request<tonic::Streaming<ConnectRequest>>,
            ...
            let Some(connect_request::Payload::Hello(hello)) = first.payload else {
            ...
            response_tx
                .send(Ok(ConnectResponse {
                    payload: Some(connect_response::Payload::ConnectAck(ConnectAck {
            ...
            tokio::spawn(async move {
                for dispatch in &shared.dispatches {
                    if response_tx
                        .send(Ok(ConnectResponse {
                            payload: Some(connect_response::Payload::CommandDispatch(
                                dispatch.clone(),
                            )),
                        }))
                        .await
                ...
                loop {
                    match inbound.message().await {
                        Ok(Some(frame)) => match frame.payload {
                            Some(connect_request::Payload::Heartbeat(heartbeat)) => {
                                let _ = shared.heartbeat_tx.send(heartbeat);
                                if shared.ack_heartbeats {
                                    let _ = response_tx
                                        .send(Ok(ConnectResponse {
                                            payload: Some(connect_response::Payload::HeartbeatAck(
                                                HeartbeatAck {
                            ...
                            Some(connect_request::Payload::CommandResult(result)) => {
                                let _ = shared.command_result_tx.send(result);
                            }
                            Some(connect_request::Payload::Hello(_)) | None => return,
                        },
